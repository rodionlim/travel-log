...
enum class DocumentHint(val label: String) {
    ...
    fun promptAddendum(): String = when (this) {
        ...
        CAR_RENTAL -> "The user says this document is a CAR RENTAL booking. Return it as an activity with title like 'Car rental – <company>', use dateTime for pickup, endDateTime for dropoff, keep pickupLocation and dropoffLocation as separate fields, include total price and booking reference when visible, and keep vehicle or rental details in notes. Return activities only unless other categories are unambiguously present."
        ACTIVITY -> "The user says this document is an ACTIVITY, EXCURSION, TOUR, or RESTAURANT booking. Focus extraction on activity title, location, start date/time, end date/time when visible, total price when visible, and booking reference when visible. Return activities only unless other categories are unambiguously present."
